// Shared animation core functions used by both renderer-page.html and web app
...
// Animation phase execution functions
export async function runZoomPhase(map, encoder, program, fps, duration, frameCount, ease) {
...
export async function runWaitPhase(map, encoder, ms, fps) {
...
export async function runTracePhase(map, encoder, program, fps) {
  console.log("tracing border")
  const baseDuration = program.border?.traceDurationMs ?? 3000;
  const fastMode = program.animation?.fastMode ?? false;
  const traceMs = fastMode ? Math.min(baseDuration * 0.5, 1500) : baseDuration; // 50% faster in fast mode, max 1.5s
  ...
  for (let j = 0; j < traceFrames; j += frameSkip) {
    ...
    // In fast mode, use requestAnimationFrame instead of waiting for idle for server-side rendering
    if (fastMode && !encoder) {
      await new Promise(res => requestAnimationFrame(res));
    } else {
...
export async function runHoldPhase(map, encoder, program, fps) {
...
// Preview animation runner (for web app - no encoder)
export async function runPreviewAnimation(map, program) {
  if (!program?.camera?.keyframes) return;
  ...
  return new Promise(resolve => {
    const tick = () => {
      ...
      const [i, j] = findSpan(program.camera.keyframes, elapsed);
      const a = program.camera.keyframes[i], b = program.camera.keyframes[j];
      const tt = a.t === b.t ? 0 : (elapsed - a.t) / (b.t - a.t);
      const p = lerpFrame(a, b, tt);
      map.jumpTo({ center: p.center, zoom: p.zoom, bearing: p.bearing, pitch: p.pitch });
...
export async function runAnimationSequence(map, program) {
  ...
  const easeName = program.animation?.easing || 'easeOutCubic';
  const ease = EASING[easeName] || EASING.easeOutCubic;
  const fps = 30; // preview fps
  const frameCount = Math.ceil((duration / 1000) * fps);

  console.log("Running animation phases:", phases);
  
  for (const phase of phases) {
    console.log("Running phase:", phase);
    if (phase === 'zoom') await runZoomPhase(map, null, program, fps, duration, frameCount, ease);
    else if (phase === 'wait') await runWaitPhase(map, null, Number(program.animation?.waitBeforeTraceMs || 0), fps);
    else if (phase === 'trace') await runTracePhase(map, null, program, fps);
    else if (phase === 'hold') await runHoldPhase(map, null, program, fps);
  }
}
