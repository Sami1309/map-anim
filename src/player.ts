...
export class MapAnimPlayer {
  ...
  private inferOrientationFromContainer(): '16:9' | '9:16' {
    try {
      const w = (this.container as HTMLElement)?.clientWidth || this.lastSize?.w || 960
      const h = (this.container as HTMLElement)?.clientHeight || this.lastSize?.h || 540
      if (!h) return '16:9'
      return (w / h) >= 1 ? '16:9' : '9:16'
    } catch { return '16:9' }
  }

  private async prepareForProgramForRecord(program: any) {
    ...
    const close = (a: number, b: number) => Math.abs(a - b) / Math.max(1, Math.abs(b)) < 0.02

    // Optional orientation hint support: 'portrait' | 'landscape' | '9:16' | '16:9'
    let orientHint: string | undefined = (p.output.orientation || p.output.aspect || p.output.ratio)
      ? String(p.output.orientation || p.output.aspect || p.output.ratio).toLowerCase()
      : undefined
    let desiredOrient: '16:9' | '9:16' | undefined
    if (orientHint) {
      if (orientHint.includes('9:16') || orientHint.includes('portrait') || orientHint === '9x16') desiredOrient = '9:16'
      else if (orientHint.includes('16:9') || orientHint.includes('landscape') || orientHint === '16x9') desiredOrient = '16:9'
    }
    if (!desiredOrient && hasDims) desiredOrient = (ow / oh) >= 1 ? '16:9' : '9:16'
    if (!desiredOrient) desiredOrient = this.inferOrientationFromContainer()
    ...
    try { console.log('[player] record: using output size', p.output.width, 'x', p.output.height, 'orient=', (p.output.width >= p.output.height ? '16:9' : '9:16')); } catch {}
  ...
  async play(program: ResolvedProgram, opts?: PlayOptions & { onPhaseStart?: (name: string)=>void; onPhaseEnd?: (name: string)=>void; signal?: AbortSignal }, _bypassRecordGuard: boolean = false) {
    if (!this.map) throw new Error('player_not_initialized');
    if (this._isRecording && !_bypassRecordGuard) { try { console.warn('[player] play() suppressed during recording'); } catch {} return; }
  ...
  async seek(program: any, tMs: number, _bypassRecordGuard: boolean = false) {
    if (!this.map) throw new Error('player_not_initialized');
    if (this._isRecording && !_bypassRecordGuard) { try { console.warn('[player] seek() suppressed during recording'); } catch {} return; }
    if (!program) return;
    this.lastProgram = program;
  ...
  preview(opts: { program?: any; timeMs: number }, _bypassRecordGuard: boolean = false) {
    if (!this.map) throw new Error('player_not_initialized');
    if (this._isRecording && !_bypassRecordGuard) { try { console.warn('[player] preview() suppressed during recording'); } catch {} return; }
  ...
  setTime(ms: number, _bypassRecordGuard: boolean = false) {
    if (this._isRecording && !_bypassRecordGuard) { try { console.warn('[player] setTime() suppressed during recording'); } catch {} return; }
    return this.preview({ timeMs: ms }, _bypassRecordGuard)
  }

  setProgress(p: number, _bypassRecordGuard: boolean = false) {
  ...
  async record(program: ResolvedProgram, opts?: RecordOptions & { signal?: AbortSignal }): Promise<Blob> {
    ...
    try {
      ...
      const fps = Number((activeProgram as any)?.output?.fps || 30);
      ...
      const totalMs = this.getDuration(activeProgram as any);
      await new Promise<void>((resolve, reject) => {
        let stopped = false;
        const stopAll = () => { if (stopped) return; stopped = true; try { rec.stop(); } catch {} };
        ...
        const step = () => {
          ...
          if (elapsed >= totalMs) { stopAll(); return; }
