// Browser-compatible version of animation core functions
...
// Animation phase execution functions (browser versions - no encoder parameter)
export async function runZoomPhase(map, program, fps, duration, frameCount, ease) {
...
export async function runWaitPhase(map, ms, fps) {
...
export async function runTracePhase(map, program, fps) {
  ...
  for (let j = 0; j < traceFrames; j += frameSkip) {
    ...
    // In fast mode, use requestAnimationFrame instead of waiting for idle
    if (fastMode) {
      await new Promise(res => requestAnimationFrame(res));
    } else {
...
export async function runHoldPhase(map, program, fps) {
...
export async function runAnimationSequence(map, program) {
  if (!program?.camera?.keyframes || program.camera.keyframes.length === 0) {
    console.warn("No camera keyframes found, skipping animation");
    return;
  }
  ...
  console.log("Running animation phases:", phases, "duration:", duration, "frames:", frameCount);
  
  for (const phase of phases) {
    console.log("Running phase:", phase);
    try {
      if (phase === 'zoom') {
        await runZoomPhase(map, program, fps, duration, frameCount, ease);
      } else if (phase === 'wait') {
        await runWaitPhase(map, Number(program.animation?.waitBeforeTraceMs || 0), fps);
      } else if (phase === 'trace') {
        // Ensure border is set up before tracing
        if (program.border?.isoA3) {
          await runTracePhase(map, program, fps);
        } else {
          console.warn("Skipping trace phase - no border ISO3 specified");
        }
      } else if (phase === 'hold') {
        await runHoldPhase(map, program, fps);
      }
    } catch (error) {
