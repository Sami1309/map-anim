...
app.post("/api/resolve", async (req, res) => {
    try {
        ...
        try {
            ...
            const hasIso = program?.border?.isoA3;
            const already = program?.boundaryGeoJSON;
            if (wantHigh && hasIso && !already) {
                const iso3 = String(program.border.isoA3 || '').toUpperCase();
                try {
                    ...
                    if (r.ok) {
                        ...
                        try {
                            const fc = topojson.feature?.(topo, topo.objects?.countries) || null;
                            if (fc && fc.features) {
                                const feats = fc.features.filter((f) => {
                                    const p = f.properties || {};
                                    const candidates = [p.ADM0_A3, p.ISO_A3, p['ISO3166-1-Alpha-3'], p.iso_a3];
                                    return candidates.map((x) => (x || '').toString().toUpperCase()).includes(iso3);
                                });
                                if (feats.length) {
                                    program.boundaryGeoJSON = { type: 'FeatureCollection', features: feats };
                                }
                            }
                        }
                        catch { }
...
async function augmentProgram(prog, text) {
    ...
    function computeZoomForBbox(bbox, width, height, paddingPx = 80, tileSize = 512) {
        try {
            ...
            if (lonDelta <= 0)
                lonDelta = 360; // fallback if broken
    ...
    if (p.extras?.address || (text && looksLikeAddress(text))) {
        try {
            ...
            if (addr) {
                ...
                didAddressZoom = true;
                const padding = (p?.animation?.fitPaddingPx ?? 80);
                ...
                if (!p.camera.keyframes?.length) {
                    ...
                    const last = p.camera.keyframes[p.camera.keyframes.length - 1];
                    last.center = [lon, lat];
                    if (typeof last.zoom !== 'number' || !Number.isFinite(last.zoom))
                        last.zoom = finalZoom;
                    else
                ...
                try {
                    console.log('[augment] address keyframes preview:', p.camera.keyframes);
                }
                catch { }
            }
        }
    ...
    if (Array.isArray(p.segments) && p.segments.length) {
        ...
        try {
            const traceOnly = p.segments.filter((s) => Array.isArray(s.phases) && s.phases.includes('trace'));
            const pick = (traceOnly.length === 1 ? traceOnly[0] : (p.segments.length === 1 ? p.segments[0] : null));
            if (!p.boundaryGeoJSON && pick?.boundaryGeoJSON) {
                p.boundaryGeoJSON = pick.boundaryGeoJSON;
